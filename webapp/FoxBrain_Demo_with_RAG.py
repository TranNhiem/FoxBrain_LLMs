...
prompt_extract = f"你是一位愛台灣的人工智慧助手，現在是{currentYear}年{currentMonth}月{currentDay}日。\
您的任務是依據使用者的訊息，從文章中搜尋相關資訊，並整理後以第三人稱回覆給使用者。\
回覆前，請先判斷文章中有否足夠的資訊可以來回答使用者，\
如果可以，請務必提供詳盡且完整的說明，並以以下格式回覆：<<是>><找到的相關資訊>。如果文章資訊不足，請以以下格式回覆：<<否>>。"

prompt_query = f"你是一位愛台灣的人工智慧助手，現在是{currentYear}年{currentMonth}月{currentDay}日。\
您的任務是根據使用者的訊息，來理解使用者的問題，並利用 Google 搜尋以找到相關答案。\
若您能提供相關搜尋問題，請使用以下格式：<<是>><放入Google搜尋的問題>。\
若無法提供相應的搜尋，請以以下格式回覆：<<否>>。"

prompt_summ = f"你是一位愛台灣的人工智慧助手，現在是{currentYear}年{currentMonth}月{currentDay}日。\
您的任務是自然流暢地回應使用者訊息。在回應時，請先查閱Google搜尋結果。\
如果搜尋結果有幫助，請以以下格式回覆：<<是>><您自然流暢的回應>。\
若搜尋結果無法提供幫助，請以以下格式回覆：<<否>>。"
        ...
        for url in results:
            if success < 5:
                try:
                    ...
                    input_to_bot = f"{message}\n\n文章:\n{article.text[:1800]}"
                    print('*********'*5,f"\ninput_to_extract_bot:\n{input_to_bot}\n",'*********'*5)
                    extract_info = bot.generate(message=input_to_bot)
                    print('*********'*5,f"\noutput_of_extract_bot:\n{extract_info}\n",'*********'*5)
                    if ("<否>" in extract_info) or (extract_info == ''):# check if useful
                        print(f'web content not useful or empty extraction output:\n{url}')
                    else:
                        success += 1
